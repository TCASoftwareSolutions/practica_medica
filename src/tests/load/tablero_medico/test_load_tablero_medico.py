...
class TestTableroMedico:
    ...
    logger_dir = f"load/tablero_medico/tablero_medico_{logger_date}/tablero_medico_{logger_datetime}"
    screenshot_dir = f"load/tablero_medico/tablero_medico_{logger_date}/tablero_medico_{logger_datetime}"
    ...
    def test01_Open_Program_Medico(self, ui_adapter: UIPort, record_pipelines_property):
        ...
        if not agenda_medica.check_nueva_cita_asignada():
            ...
            agenda_medica.onClick_AbrirModulo()
        ...
        tablero_medico.set_button_diagnostico_principal("5A11 - Diabetes mellitus tipo 2")
        # tablero_medico.set_diagnostico_principal("")
